...
const Home = ({ state, account }) => {
  ...
  const loadPosts = async () => {
    ...
    let posts = await Promise.all(
      results.map(async (i) => {
        let response = await fetch(`https://ipfs.io/ipfs/${i[1]}`); //image ipfs se fetch ki
        const metadataPost = await response.json(); //image ke meta data ko store kiya hai json me

        const nftId = await contract.profiles(i.author); //author ki id hai

        const uri = await contract.tokenURI(nftId); // uri of nft profile
  ...
  const uploadPost = async () => {
    ...
    try {
      ...
      console.log(result);
      setLoading(true);
      await (await contract.uploadPost(result.data.IpfsHash)).wait();
      loadPosts();
    } catch (error) {
      window.alert(error);
    }
  ...
  const tip = async (post) => {
    await (
      await contract.tipPostOwner(post.id, { value: ethers.parseEther("0.1") })
    ).wait();
    loadPosts();
  };
              ...
              <Card border="primary">
                ...
                <Card.Footer className="list-group-item">
                  <div className="d-inline mt-auto float-start">
                    Tip Amount: {ethers.formatEther(post.tipAmount)} ETH
                  </div>
